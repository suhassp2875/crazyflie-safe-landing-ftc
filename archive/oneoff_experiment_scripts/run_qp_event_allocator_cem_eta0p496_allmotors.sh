...
ETA=0.496
WEIGHT_CONFIG="${WEIGHT_CONFIG:-configs/allocator_weights/cem_tuned_boundary.json}"
REPS=(1 2 3 4 5)
...
run_one_trial() {
    ...
    local rep="$2"
    local eta_tag="${ETA/./p}"
    local tag="cem_tuned_m${motor}_eta${eta_tag}_rep${rep}"

    echo "============================================================"
    echo "[CEM-TUNED QP EVENT ALLOCATOR ETA0.496] motor=${motor}, rep=${rep}"
    ...
    cd "$CF_FW_DIR" || exit 1
    bash "$SIM_LAUNCH" -m crazyflie -x 0 -y 0 > "$PROJECT_DIR/logs/sim_cem_qpevent_eta0p496_m${motor}_rep${rep}.log" 2>&1 &
    sleep 8
...
echo "[DONE] CEM-tuned state-aware QP event allocator eta=0.496 all-motor sweep completed."
