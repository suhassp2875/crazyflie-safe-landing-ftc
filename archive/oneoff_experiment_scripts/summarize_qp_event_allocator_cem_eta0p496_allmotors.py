...
pat = re.compile(r"qp_event_allocator_m(\d+)_eta0p496_cem_tuned_m\d+_eta0p496_rep(\d+)\.csv")

for path in sorted(Path("logs").glob("qp_event_allocator_m*_eta0p496_cem_tuned_m*_eta0p496_rep*.csv")):
    ...
    rows.append({
        "controller": "cem_tuned_state_aware_qp_event_allocator",
        "allocator_config": str(row.get("allocator_config", "")),
        "motor": motor,
        "eta": 0.496,
...
if out.empty:
    raise SystemExit("[ERROR] No valid CEM-tuned eta=0.496 logs found.")

out = out.sort_values(["motor", "rep"])
out_path = Path("results/tables/qp_event_allocator_cem_eta0p496_allmotors_summary.csv")
out_path.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(out_path, index=False)

print("\n[CEM-TUNED STATE-AWARE QP EVENT ALLOCATOR ETA=0.496 ALL MOTORS]")
print(out.to_string(index=False))

agg = out.groupby(["controller", "allocator_config", "motor", "eta", "candidate", "r1", "r2", "r3", "r4"]).agg(
    n=("rep", "count"),
...
agg_path = Path("results/tables/qp_event_allocator_cem_eta0p496_allmotors_aggregate.csv")
...
plt.title("CEM-tuned state-aware QP event allocator, eta=0.496")
plt.grid(True, axis="y")
plt.legend()

fig_path = Path("results/figures/qp_event_allocator_cem_eta0p496_allmotors.png")
