...
def send_adaptive_landing_v2(cf, logger, start_z, end_z, duration_s):
    """
    Ground-aware adaptive emergency landing controller.

    Key design:
    - descend slowly after fault
    - if falling too fast, command slight upward brake
    - clamp z_cmd so it cannot climb unrealistically
    - near ground, never command high altitude again
    - terminate landing loop after ground contact is detected
    """
    dt = 1.0 / COMMAND_RATE_HZ
    steps = int(duration_s * COMMAND_RATE_HZ)
    ...
    nominal_descent_rate = 0.045   # m/s
    near_ground_rate = 0.018       # m/s

    # Velocity thresholds
    fast_fall_vz = -0.24
    moderate_fall_vz = -0.16
    ...
    for _ in range(steps):
        ...
        # If we have reached near ground, stop descending further.
        # This avoids the rebound seen in adaptive v1.
        if z == z and z <= GROUND_CONTACT_Z_M:
            touchdown_detected = True
            z_cmd = min(z_cmd, end_z)
        ...
        # Descent schedule
        if z == z and z < 0.18:
            descent_rate = near_ground_rate
        else:
            descent_rate = nominal_descent_rate

        # Braking logic if falling too fast.
        if vz < fast_fall_vz:
            # Ask for a little more altitude, but clamp relative to current z.
            if z == z:
                z_cmd = min(z_cmd + 0.008, z + 0.10, start_z)
            else:
                z_cmd = min(z_cmd + 0.008, start_z)

        elif vz < moderate_fall_vz or tilt > 8.0:
            # Hold target, do not keep descending.
            z_cmd = z_cmd
        ...
        # Ground-aware clamps to avoid v1 rebound.
        if z == z and z < 0.15:
            z_cmd = min(z_cmd, 0.15)
        if z == z and z < 0.08:
            z_cmd = min(z_cmd, 0.08)
        if z == z and z < 0.05:
            z_cmd = min(z_cmd, 0.04)

        z_cmd = max(end_z, min(z_cmd, start_z))
        logger.latest["z_cmd"] = z_cmd

        cf.commander.send_hover_setpoint(0.0, 0.0, 0.0, z_cmd)
        time.sleep(dt)

    print(f"[ADAPTIVE_V2] touchdown_detected={touchdown_detected}, final z_cmd={z_cmd:.3f}")
...
def main():
    ...
    if args.tag is None:
        log_tag = f"adaptive_v2_m{fault_motor}_eta{fault_eta:.2f}".replace(".", "p")
    else:
        log_tag = args.tag
    ...
    with SyncCrazyflie(URI) as scf:
        ...
        try:
            ...
            print(f"[CMD] adaptive V2 emergency landing: start={TAKEOFF_Z_M:.2f} m, target={FINAL_Z_M:.2f} m, duration={EMERGENCY_LANDING_S:.1f} s")
            send_adaptive_landing_v2(cf, logger, TAKEOFF_Z_M, FINAL_Z_M, EMERGENCY_LANDING_S)
