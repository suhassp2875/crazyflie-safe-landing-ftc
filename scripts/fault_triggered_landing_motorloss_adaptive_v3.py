...
def send_adaptive_landing_v3(cf, logger, start_z, end_z, duration_s):
    """
    Adaptive V3: predictive braking emergency landing.

    Main idea:
    - Do not blindly ramp altitude down after the fault.
    - If downward velocity grows too large while still above ground,
      command a higher z target to brake the fall.
    - Once vertical speed is controlled, descend slowly again.
    - Stop the landing loop once first ground contact is detected.
    """
    dt = 1.0 / COMMAND_RATE_HZ
    steps = int(duration_s * COMMAND_RATE_HZ)
    ...
    # Conservative descent rates
    high_alt_descent_rate = 0.035
    low_alt_descent_rate = 0.012

    for _ in range(steps):
        z = logger.latest.get("z", float("nan"))
        vz = logger.latest.get("vz", 0.0)
        roll = abs(logger.latest.get("roll_deg", 0.0))
        pitch = abs(logger.latest.get("pitch_deg", 0.0))
        tilt = max(roll, pitch)

        # Stop once first contact is detected. The evaluator will use this row.
        if z == z and z <= GROUND_CONTACT_Z_M:
            touchdown_detected = True
            logger.latest["z_cmd"] = z_cmd
            cf.commander.send_hover_setpoint(0.0, 0.0, 0.0, max(end_z, min(z_cmd, 0.08)))
            time.sleep(dt)
            break

        # Strong predictive braking before impact.
        # This is the key difference from V2.
        if z == z and z < 0.35 and vz < -0.22:
            # Brake hard while still above the ground.
            z_cmd = min(max(z_cmd, z + 0.22), 0.55)

        elif z == z and z < 0.22 and vz < -0.16:
            # Medium braking near ground.
            z_cmd = min(max(z_cmd, z + 0.16), 0.35)

        elif vz < -0.28:
            # Falling very fast at any altitude: pause descent and brake.
            if z == z:
                z_cmd = min(max(z_cmd, z + 0.18), start_z + 0.05)
            else:
                z_cmd = min(z_cmd + 0.02, start_z + 0.05)

        elif vz < -0.18 or tilt > 8.0:
            # Hold target until vehicle settles.
            z_cmd = z_cmd

        else:
            # Controlled slow descent.
            if z == z and z < 0.25:
                descent_rate = low_alt_descent_rate
            else:
                descent_rate = high_alt_descent_rate

            z_cmd = max(end_z, z_cmd - descent_rate * dt)

        # Clamp command. Unlike V2, we allow a braking target above current z
        # near ground, but not high enough to cause a huge rebound.
        z_cmd = max(end_z, min(z_cmd, start_z + 0.05))

        logger.latest["z_cmd"] = z_cmd
        cf.commander.send_hover_setpoint(0.0, 0.0, 0.0, z_cmd)
        time.sleep(dt)

    print(f"[ADAPTIVE_V3] touchdown_detected={touchdown_detected}, final z_cmd={z_cmd:.3f}")
...
def main():
    ...
    if args.tag is None:
        log_tag = f"adaptive_v3_m{fault_motor}_eta{fault_eta:.2f}".replace(".", "p")
    else:
        log_tag = args.tag
    ...
    with SyncCrazyflie(URI) as scf:
        ...
        try:
            ...
            print(f"[CMD] adaptive V3 emergency landing: start={TAKEOFF_Z_M:.2f} m, target={FINAL_Z_M:.2f} m, duration={EMERGENCY_LANDING_S:.1f} s")
            send_adaptive_landing_v3(cf, logger, TAKEOFF_Z_M, FINAL_Z_M, EMERGENCY_LANDING_S)
