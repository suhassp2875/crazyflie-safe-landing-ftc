...
ROOT="$PROJECT_DIR/results/final/model_sensitivity/ofat/pinv_boundary_fine_sweep"
SCHEDULE="$ROOT/ofat_fine_schedule.csv"
...
# PILOT=1 selects fresh seed rep=1 at the
# lower and upper endpoint for every condition.
PILOT="${PILOT:-0}"
...
validate_trial() {
  ...
  python \
    ...
    --ofat-protocol-id crazyflie_ofat_pinv_fine_v1 \
...
run_trial() {
  ...
  local eta_prefix
  eta_prefix=$(eta_prefix_tag "$eta")

  local eta_unique
  eta_unique=$(eta_unique_tag "$eta")

  local factor_file_tag
  factor_file_tag=$(factor_tag "$factor")

  local tag
  tag="ofat_fine_${condition_id}_f${factor_file_tag}_eta${eta_unique}_seed${seed}"

  local generated_csv
  generated_csv="$PROJECT_DIR/logs/qp_event_allocator_m2_eta${eta_prefix}_${tag}.csv"
...
if len(rows) != 2100:
    raise SystemExit(
        f"[FAIL] Expected 2100 schedule rows; "
        f"found {len(rows)}."
    )
...
if set(condition_counts.values()) != {210}:
    raise SystemExit(
        "[FAIL] Every condition must contain "
        "210 scheduled trials."
    )
...
for row in rows:
    ...
    if (
        row["ofat_protocol_id"]
        != "crazyflie_ofat_pinv_fine_v1"
    ):
        raise SystemExit(
            "[FAIL] Unexpected OFAT protocol."
        )
...
for condition_id, seeds in seed_sets.items():
    ...
    if len(eta_sets[condition_id]) != 7:
        raise SystemExit(
            f"[FAIL] {condition_id} does not "
            "contain seven eta points."
        )
...
if [ "$PILOT" -eq 1 ]; then
  python \
    scripts/summarize_ofat_m2_pinv_fine_sweep.py \
    --root "$ROOT" \
    --allow-incomplete
