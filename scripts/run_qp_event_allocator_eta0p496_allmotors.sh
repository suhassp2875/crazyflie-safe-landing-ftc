...
run_one_trial() {
    ...
    echo "============================================================"
    echo "[QP EVENT ALLOCATOR ETA0.496] motor=${motor}, rep=${rep}"
    echo "============================================================"

    cleanup_sim

    cd "$CF_FW_DIR" || exit 1
    bash "$SIM_LAUNCH" -m crazyflie -x 0 -y 0 > "$PROJECT_DIR/logs/sim_qpevent_eta0p496_m${motor}_rep${rep}.log" 2>&1 &
    sleep 8
...
echo "[DONE] State-aware QP event allocator eta=0.496 all-motor sweep completed."
