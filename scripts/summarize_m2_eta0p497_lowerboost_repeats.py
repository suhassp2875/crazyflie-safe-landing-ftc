...
for path in sorted(Path("logs").glob("motorloss_ftcboost_m2_eta0p497_b*_r0_0_0_0_lower_rep*.csv")):
    m = re.search(r"m2_eta0p497_b(\d+)_r0_0_0_0_lower_rep(\d+)", path.name)
    if not m:
        continue
...
out_path = Path("results/tables/m2_eta0p497_lowerboost_repeats_summary.csv")
out_path.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(out_path, index=False)

print("\n[M2 ETA=0.497 LOWER BOOST REPEATS]")
...
agg_path = Path("results/tables/m2_eta0p497_lowerboost_repeats_aggregate.csv")
...
plt.title("Motor 2 eta=0.497: lower equal-boost search")
plt.grid(True)
plt.legend()

fig_path = Path("results/figures/m2_eta0p497_lowerboost_repeats.png")
