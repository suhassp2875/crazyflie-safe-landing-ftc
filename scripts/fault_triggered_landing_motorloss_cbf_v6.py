...
def send_cbf_backup_landing_v6(cf, logger, start_z, end_z, duration_s, fault_eta):
    """
    CBF V6: fault-triggered backup safety filter.

    Why V6:
    - V4/V5 waited for vertical motion to become dangerous.
    - Max-brake proved eta=0.50 is recoverable if braking starts immediately.
    - Therefore, the filter must use fault severity, not only z and vz.

    Nominal controller:
        fixed ramp descent.

    Safety filter:
        if eta is inside the unsafe ramp region, latch backup immediately.
        otherwise use landing-feasibility h_land.

    Backup controller:
        max-brake z_cmd = 0.95.
    """
    dt = 1.0 / COMMAND_RATE_HZ
    steps = int(duration_s * COMMAND_RATE_HZ)
    ...
    # Boundary found from baseline sweep:
    # fixed ramp safe at eta=0.52, unsafe at eta=0.50.
    eta_brake_threshold = 0.52
    ...
    # Fault-triggered latch.
    # This is the key difference from V4/V5.
    if fault_eta <= eta_brake_threshold:
        cbf_latched = True
        print(
            f"[CBF_V6] fault-triggered backup latched immediately: "
            f"eta={fault_eta:.3f} <= threshold={eta_brake_threshold:.3f}"
        )
    ...
    for k in range(steps):
        ...
        # State-triggered backup for eta values above threshold.
        # This remains as a secondary filter.
        trigger_backup = False

        if h_land < 0.0:
            trigger_backup = True

        if z == z and z < 0.55 and vz < -0.03:
            trigger_backup = True
    ...
    print(
        f"[CBF_V6] touchdown_detected={touchdown_detected}, "
        f"brake_z_cmd={brake_z_cmd:.3f}, cbf_activation_count={cbf_activation_count}, "
        f"cbf_latched={cbf_latched}"
    )
...
def main():
    ...
    if args.tag is None:
        log_tag = f"cbf_v6_m{fault_motor}_eta{fault_eta:.2f}".replace(".", "p")
    else:
        log_tag = args.tag
    ...
    with SyncCrazyflie(URI) as scf:
        ...
        try:
            ...
            print(f"[CMD] CBF V6 fault-triggered backup-filter landing: start={TAKEOFF_Z_M:.2f} m, target={FINAL_Z_M:.2f} m, duration={EMERGENCY_LANDING_S:.1f} s")
            send_cbf_backup_landing_v6(cf, logger, TAKEOFF_Z_M, FINAL_Z_M, EMERGENCY_LANDING_S, fault_eta)
