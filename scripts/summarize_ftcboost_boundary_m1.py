...
def main():
    ...
    for path in sorted(Path("logs").glob("motorloss_ftcboost_m1_eta*_b*_r0_0_0_0.csv")):
        ...
        # Focus on boundary sweep range
        if eta < 0.45 or eta > 0.50:
            continue
        if boost not in [5000, 7000, 10000]:
            continue
    ...
    if not rows:
        print("[ERROR] No boundary logs found.")
        return

    out = pd.DataFrame(rows).sort_values(["eta", "boost"])
    out_path = Path("results/tables/ftcboost_boundary_m1_summary.csv")
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out.to_csv(out_path, index=False)

    print("\n[FTC BOOST BOUNDARY: MOTOR 1]")
    print(out[[
        "eta",
        "boost",
        "vertical_speed_mps",
    ...
    plt.title("Recoverability boundary: motor 1 FTC boost")
    plt.grid(True)
    plt.legend()

    fig_path = Path("results/figures/ftcboost_boundary_m1.png")
    fig_path.parent.mkdir(parents=True, exist_ok=True)
    plt.tight_layout()
    plt.savefig(fig_path, dpi=200)

    # Minimum safe eta per boost
    safe = out[out["safe_touchdown"] == True].copy()
    if len(safe):
        min_eta = safe.sort_values(["boost", "eta"]).groupby("boost").first().reset_index()
        print("\n[MINIMUM SAFE ETA PER BOOST]")
        print(min_eta[[
            "boost",
            "eta",
