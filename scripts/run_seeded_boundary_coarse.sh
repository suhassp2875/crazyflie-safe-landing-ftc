...
ETAS="${ETAS:-0.44 0.45 0.46 0.47 0.48 0.49 0.50}"
REPS="${REPS:-12}"
...
PY2
    ...
    cd "$CF_FW_DIR" || exit 1
    bash "$SIM_LAUNCH" -m crazyflie -x "$SPAWN_X" -y "$SPAWN_Y" > "$PROJECT_DIR/logs/sim_seededcoarse_${controller}_m${motor}_eta${eta_tag}_seed${seed}.log" 2>&1 &
    sleep 8
