...
pat = re.compile(r"qp_event_allocator_m(\d+)_eta0p496_m\d+_eta0p496_rep(\d+)\.csv")

for path in sorted(Path("logs").glob("qp_event_allocator_m*_eta0p496_m*_eta0p496_rep*.csv")):
...
if out.empty:
    raise SystemExit("[ERROR] No valid state-aware QP eta=0.496 logs found.")

out = out.sort_values(["motor", "rep"])
out_path = Path("results/tables/qp_event_allocator_eta0p496_allmotors_summary.csv")
out_path.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(out_path, index=False)

print("\n[STATE-AWARE QP EVENT ALLOCATOR ETA=0.496 ALL MOTORS]")
...
agg_path = Path("results/tables/qp_event_allocator_eta0p496_allmotors_aggregate.csv")
...
plt.title("State-aware QP event allocator, eta=0.496")
plt.grid(True, axis="y")
plt.legend()

fig_path = Path("results/figures/qp_event_allocator_eta0p496_allmotors.png")
