...
def send_adaptive_landing(cf, logger, start_z, end_z, duration_s):
    """
    Adaptive emergency landing controller.

    Instead of blindly ramping z_cmd down, this controller watches the
    estimated vertical velocity. If the vehicle is falling too fast, it
    pauses or raises the altitude command slightly to demand more thrust.

    This is a non-RL adaptive baseline.
    """
    dt = 1.0 / COMMAND_RATE_HZ
    steps = int(duration_s * COMMAND_RATE_HZ)

    z_cmd = start_z
    nominal_descent_rate = 0.085  # m/s, slower than fixed ramp near boundary
    slow_descent_rate = 0.035     # m/s near ground or during fast fall

    max_cmd_z = start_z + 0.15
    ...
    for _ in range(steps):
        ...
        # Near ground, descend more slowly.
        if z == z and z < 0.18:
            descent_rate = slow_descent_rate
        else:
            descent_rate = nominal_descent_rate

        # If falling too fast, stop lowering the target.
        if vz < -0.25:
            z_cmd = min(z_cmd + 0.015, max_cmd_z)

        # If moderately fast fall or attitude disturbance, hold current target.
...
def main():
    ...
    if args.tag is None:
        log_tag = f"adaptive_m{fault_motor}_eta{fault_eta:.2f}".replace(".", "p")
    else:
        log_tag = args.tag
    ...
    with SyncCrazyflie(URI) as scf:
        ...
        try:
            ...
            print(f"[CMD] adaptive emergency landing: start={TAKEOFF_Z_M:.2f} m, target={FINAL_Z_M:.2f} m, duration={EMERGENCY_LANDING_S:.1f} s")
            send_adaptive_landing(cf, logger, TAKEOFF_Z_M, FINAL_Z_M, EMERGENCY_LANDING_S)
