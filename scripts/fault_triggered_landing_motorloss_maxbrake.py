...
def send_max_brake_hold(cf, logger, start_z, end_z, duration_s):
    """
    Recoverability test.

    After motor fault, do not descend. Command a high altitude target and check
    whether the damaged vehicle can arrest descent using the hover commander.
    """
    dt = 1.0 / COMMAND_RATE_HZ
    steps = int(duration_s * COMMAND_RATE_HZ)

    # Strong but not insane altitude command.
    z_cmd = min(start_z + 0.25, 0.95)

    for _ in range(steps):
        z = logger.latest.get("z", float("nan"))

        # If ground contact is reached, stop the test loop.
    ...
    print(f"[MAX_BRAKE] commanded z_cmd={z_cmd:.3f}")
...
def main():
    ...
    if args.tag is None:
        log_tag = f"maxbrake_m{fault_motor}_eta{fault_eta:.2f}".replace(".", "p")
    else:
        log_tag = args.tag
    ...
    with SyncCrazyflie(URI) as scf:
        ...
        try:
            ...
            print(f"[CMD] max-brake hold after fault: command high altitude for {EMERGENCY_LANDING_S:.1f} s")
            send_max_brake_hold(cf, logger, TAKEOFF_Z_M, FINAL_Z_M, EMERGENCY_LANDING_S)
