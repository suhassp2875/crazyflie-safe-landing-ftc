...
pat = re.compile(
    r"motorloss_ftcboost_m(\d+)_eta0p497_b0_r(-?\d+)_(-?\d+)_(-?\d+)_(-?\d+)_tuned_eta0p497_rep(\d+)\.csv"
)

for path in sorted(Path("logs").glob("motorloss_ftcboost_m*_eta0p497_b0_r*_tuned_eta0p497_rep*.csv")):
...
if out.empty:
    raise SystemExit("[ERROR] No tuned residual logs found.")

out = out.sort_values(["motor", "rep"])

out_path = Path("results/tables/ftcresid_tuned_eta0p497_allmotors_summary.csv")
out_path.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(out_path, index=False)

print("\n[TUNED RESIDUAL ETA=0.497 ALL MOTORS]")
print(out.to_string(index=False))

agg = out.groupby(["motor", "eta", "boost", "r1", "r2", "r3", "r4"]).agg(
    n=("rep", "count"),
...
agg_path = Path("results/tables/ftcresid_tuned_eta0p497_allmotors_aggregate.csv")
...
plt.title("Tuned residual allocation, eta=0.497, all motors")
plt.grid(True, axis="y")
plt.legend()

fig_path = Path("results/figures/ftcresid_tuned_eta0p497_allmotors.png")
