...
#   CONTROLLERS="pinv" MOTORS="2" ETAS="0.48 0.49 0.50" REPS=12 ./scripts/run_seeded_pinv_eta0p496.sh
CONTROLLERS="${CONTROLLERS:-pinv}"
...
PY2
    ...
    local eta_tag="${eta/./p}"
    local tag="seededpinv_${controller}_m${motor}_eta${eta_tag}_seed${seed}"
    ...
    echo "============================================================"
    echo "[SEEDED COARSE] controller=${controller}, motor=${motor}, eta=${eta}, rep=${rep}, seed=${seed}"
    ...
    cd "$CF_FW_DIR" || exit 1
    bash "$SIM_LAUNCH" -m crazyflie -x "$SPAWN_X" -y "$SPAWN_Y" > "$PROJECT_DIR/logs/sim_seededpinv_${controller}_m${motor}_eta${eta_tag}_seed${seed}.log" 2>&1 &
    sleep 8
...
echo "[DONE] Seeded coarse boundary sweep completed."
