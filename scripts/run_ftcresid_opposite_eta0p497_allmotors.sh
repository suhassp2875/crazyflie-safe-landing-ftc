...
run_one_trial() {
    ...
    echo "============================================================"
    echo "[OPPOSITE RESIDUAL] motor=${motor}, eta=${ETA}, rep=${rep}, r=[$r1,$r2,$r3,$r4]"
    echo "============================================================"

    cleanup_sim

    cd "$CF_FW_DIR" || exit 1
    bash "$SIM_LAUNCH" -m crazyflie -x 0 -y 0 > "$PROJECT_DIR/logs/sim_opposite_m${motor}_eta${eta_tag}_rep${rep}.log" 2>&1 &
    sleep 8
    ...
    local dst="$PROJECT_DIR/logs/motorloss_ftcboost_m${motor}_eta${eta_tag}_b${BOOST}_r${r1}_${r2}_${r3}_${r4}_opposite10000_rep${rep}.csv"
...
echo "[DONE] Opposite-motor residual all-motor eta=0.497 sweep completed."
