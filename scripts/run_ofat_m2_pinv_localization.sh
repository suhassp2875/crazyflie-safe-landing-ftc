...
ROOT="$PROJECT_DIR/results/final/model_sensitivity/ofat/pinv_boundary_localization"
SCHEDULE="$ROOT/ofat_localization_schedule.csv"
CONDITION_MANIFEST="$ROOT/ofat_condition_manifest.json"
...
# PILOT=1 selects rep=1 at the minimum and
# maximum eta for every perturbation: 20 trials.
PILOT="${PILOT:-0}"
...
if set(condition_counts.values()) != {35}:
    raise SystemExit(
        "[FAIL] Every condition must "
        "contain 35 trials."
    )

seed_sets = defaultdict(set)
etas_by_condition = defaultdict(set)
...
for condition_id, seeds in (
    ...
    if len(
        ...
        raise SystemExit(
            f"[FAIL] {condition_id} does "
            "not have seven eta points."
        )
...
if [ "$PILOT" -eq 1 ]; then
  python \
    scripts/summarize_ofat_m2_pinv_localization.py \
    --root "$ROOT" \
    --allow-incomplete
