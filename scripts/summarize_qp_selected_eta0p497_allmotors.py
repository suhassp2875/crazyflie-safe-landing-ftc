...
pat = re.compile(
    r"motorloss_ftcboost_m(\d+)_eta0p497_b0_r(-?\d+)_(-?\d+)_(-?\d+)_(-?\d+)_(.+)_qpselected_rep(\d+)\.csv"
)

for path in sorted(Path("logs").glob("motorloss_ftcboost_m*_eta0p497_b0_r*_qpselected_rep*.csv")):
    ...
    rows.append({
        "controller": "qp_selected_event_triggered",
        "motor": motor,
        "eta": 0.497,
        "candidate": candidate,
        "r1": r1,
...
if out.empty:
    raise SystemExit("[ERROR] No QP-selected logs found.")

out = out.sort_values(["motor", "rep"])
out_path = Path("results/tables/qp_selected_eta0p497_allmotors_summary.csv")
out_path.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(out_path, index=False)

print("\n[QP-SELECTED ETA=0.497 ALL MOTORS]")
...
).reset_index().sort_values(["motor"])

agg_path = Path("results/tables/qp_selected_eta0p497_allmotors_aggregate.csv")
...
print("\n[OVERALL]")
print(f"safe_count_total: {int(agg['safe_count'].sum())}/{int(agg['n'].sum())}")
...
plt.errorbar(range(len(agg)), agg["mean_vz"], yerr=agg["std_vz"].fillna(0), fmt="none", capsize=4)
...
plt.title("QP-selected residual allocation, eta=0.497")
plt.grid(True, axis="y")
plt.legend()

fig_path = Path("results/figures/qp_selected_eta0p497_allmotors.png")
