...
pat = re.compile(
    r"qp_event_allocator_m(\d+)_eta([0-9p]+)_seededfine_(qplite|cem)_m\d+_eta[0-9p]+_seed(\d+)\.csv"
)

for path in sorted(Path("logs").glob("qp_event_allocator_m*_eta*_seededfine_*_m*_eta*_seed*.csv")):
...
if out.empty:
    raise SystemExit("[ERROR] No seeded fine logs found.")

out = out.sort_values(["controller", "motor", "eta", "trial_seed"])

out_path = Path("results/tables/seeded_boundary_fine_summary.csv")
...
agg_path = Path("results/tables/seeded_boundary_fine_aggregate.csv")
...
for motor in sorted(out["motor"].unique()):
    ...
    fig_path = Path(f"results/figures/seeded_boundary_fine_motor{motor}.png")
