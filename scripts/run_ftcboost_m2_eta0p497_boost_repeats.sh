...
MOTOR=2
ETA=0.497
BOOSTS=(10000 11000 12000 13000 14000)
REPS=(1 2 3)
...
run_one_trial() {
    ...
    echo "============================================================"
    echo "[M2 ETA0.497 BOOST REPEAT] motor=${MOTOR}, eta=${ETA}, boost=${boost}, rep=${rep}"
    echo "============================================================"

    cleanup_sim

    cd "$CF_FW_DIR" || exit 1
    bash "$SIM_LAUNCH" -m crazyflie -x 0 -y 0 > "$PROJECT_DIR/logs/sim_m2_eta${eta_tag}_b${boost}_rep${rep}.log" 2>&1 &
    sleep 8
...
echo "[DONE] Motor 2 eta=0.497 boost repeat sweep completed."
