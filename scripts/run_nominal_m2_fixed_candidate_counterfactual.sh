...
ROOT="$PROJECT_DIR/results/final/model_sensitivity/nominal_m2_boundary/fixed_candidate_counterfactual"
SCHEDULE="$ROOT/fixed_candidate_schedule.csv"
...
# of the three fixed policies: three trials total.
PILOT="${PILOT:-0}"
...
validate_trial() {
  ...
  python \
    scripts/validate_nominal_m2_fixed_candidate_trial.py \
    --csv "$csv_path" \
...
run_trial() {
  ...
  local tag
  tag="nominal_m2fixed_${condition_id}_eta${file_eta_tag}_seed${seed}"
  ...
  while [ "$attempt" -le "$MAX_TRIAL_ATTEMPTS" ]; do
    ...
    python \
      ...
      --controller qplite \
      --manual-residual \
      --manual-name "$expected_candidate" \
      --r1 "$expected_r1" \
      --r2 "$expected_r2" \
      --r3 "$expected_r3" \
      --r4 "$expected_r4" \
...
if len(rows) != 90:
    raise SystemExit(
        f"[FAIL] Expected 90 schedule rows; "
        f"found {len(rows)}."
    )
...
if len(condition_counts) != 3:
    raise SystemExit(
        "[FAIL] Expected three fixed policies."
    )

if set(condition_counts.values()) != {30}:
    raise SystemExit(
        "[FAIL] Every fixed policy must "
        "contain 30 trials."
    )
...
for row in rows:
    if row["controller"] != "manual":
        raise SystemExit(
            "[FAIL] Expected controller=manual."
        )
...
if etas != {0.499675}:
    raise SystemExit(
        "[FAIL] All conditions must use "
        f"eta=0.499675; observed={etas}"
    )
...
if [ "$failures" -ne 0 ]; then
  echo \
    "[FAIL] Fixed-candidate study had $failures failed trials."
...
if [ "$PILOT" -eq 1 ]; then
  python \
    scripts/summarize_nominal_m2_fixed_candidate_counterfactual.py \
    --root "$ROOT" \
    --allow-incomplete
