...
PY2
    ...
    if [ "$trial_status" -ne 0 ]; then
        echo "[ERROR] Landing process exited with status ${trial_status}"
        return "$trial_status"
    fi

    if [ ! -s "$expected_csv" ]; then
        echo "[ERROR] Expected trial CSV was not created:"
        echo "        $expected_csv"
        return 1
    fi
