...
ETAS=(0.490 0.492 0.494 0.496 0.498 0.500)
BOOSTS=(7000 10000)
...
run_one_trial() {
    ...
    echo "============================================================"
    echo "[FTCBOOST FINE BOUNDARY] motor=${MOTOR}, eta=${eta}, boost=${boost}"
    echo "============================================================"

    cleanup_sim

    cd "$CF_FW_DIR" || exit 1
    bash "$SIM_LAUNCH" -m crazyflie -x 0 -y 0 > "$PROJECT_DIR/logs/sim_fine_boundary_m${MOTOR}_eta${eta_tag}_b${boost}.log" 2>&1 &
    sleep 8
...
echo "[DONE] Fine boundary sweep completed."
