...
run_one_trial() {
    ...
    echo "============================================================"
    echo "[FTCBOOST ALL-MOTOR REPEAT] motor=${motor}, eta=${eta}, boost=${BOOST}, rep=${rep}"
    echo "============================================================"

    cleanup_sim

    cd "$CF_FW_DIR" || exit 1
    bash "$SIM_LAUNCH" -m crazyflie -x 0 -y 0 > "$PROJECT_DIR/logs/sim_allmotor_m${motor}_eta${eta_tag}_b${BOOST}_rep${rep}.log" 2>&1 &
    sleep 8
...
for motor in "${MOTORS[@]}"; do
    for eta in "${ETAS[@]}"; do
        for rep in "${REPS[@]}"; do
            run_one_trial "$motor" "$eta" "$rep"
        done
    done
done

echo "[DONE] All-motor boundary repeat sweep completed."
