...
pat = re.compile(
    r"motorloss_ftcboost_m2_eta0p497_b0_r(-?\d+)_(-?\d+)_(-?\d+)_(-?\d+)_(.+)_repeat_rep(\d+)\.csv"
)

for path in sorted(Path("logs").glob("motorloss_ftcboost_m2_eta0p497_b0_r*_repeat_rep*.csv")):
...
if out.empty:
    raise SystemExit("[ERROR] No repeat logs found.")

out = out.sort_values(["pattern", "rep"])

out_path = Path("results/tables/ftcresid_m2_eta0p497_best_repeats_summary.csv")
out_path.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(out_path, index=False)

print("\n[M2 ETA=0.497 ASYMMETRIC BEST REPEATS]")
...
agg = agg.sort_values(
    ["safe_count", "mean_vz", "max_drift"],
    ascending=[False, True, True],
)

agg_path = Path("results/tables/ftcresid_m2_eta0p497_best_repeats_aggregate.csv")
...
plt.title("Motor 2 eta=0.497 asymmetric residual repeats")
plt.grid(True, axis="y")
plt.legend()

fig_path = Path("results/figures/ftcresid_m2_eta0p497_best_repeats.png")
