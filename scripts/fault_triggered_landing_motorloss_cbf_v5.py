...
def send_cbf_backup_landing_v5(cf, logger, start_z, end_z, duration_s):
    """
    CBF V5: latched early backup safety filter.

    Why V5:
    - V4 activated too late.
    - Max-brake proved eta=0.50 is recoverable if braking starts early.
    - Therefore, once degraded descent becomes risky, latch the backup controller.

    Nominal controller:
        fixed ramp descent.

    Safety filter:
        landing-feasibility check + early degraded-descent trigger.
    ...
    v_safe = 0.35       # touchdown vertical speed limit
    a_brake_est = 0.25  # more conservative than V4
    z_margin = 0.22     # larger margin than V4
    ...
    for k in range(steps):
        ...
        # Stopping-distance / landing-feasibility barrier
        ...
        # Standard barrier trigger
        if h_land < 0.0:
            trigger_backup = True

        # Early degraded-descent trigger.
        # V4 waited until too close to ground. This triggers while still recoverable.
        if z == z and z < 0.68 and vz < -0.035:
            trigger_backup = True

        # Stronger trigger in mid-altitude region
        if z == z and z < 0.55 and vz < -0.02:
            trigger_backup = True

        # Near-ground trigger
        if z == z and z < 0.35 and vz < -0.01:
            trigger_backup = True

        # Latch the backup controller once triggered.
        if trigger_backup:
            cbf_latched = True
    ...
    print(
        f"[CBF_V5] touchdown_detected={touchdown_detected}, "
...
def main():
    ...
    if args.tag is None:
        log_tag = f"cbf_v5_m{fault_motor}_eta{fault_eta:.2f}".replace(".", "p")
    else:
        log_tag = args.tag
    ...
    with SyncCrazyflie(URI) as scf:
        ...
        try:
            ...
            print(f"[CMD] CBF V5 latched backup-filter landing: start={TAKEOFF_Z_M:.2f} m, target={FINAL_Z_M:.2f} m, duration={EMERGENCY_LANDING_S:.1f} s")
            send_cbf_backup_landing_v5(cf, logger, TAKEOFF_Z_M, FINAL_Z_M, EMERGENCY_LANDING_S)
