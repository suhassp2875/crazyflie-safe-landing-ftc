...
pat = re.compile(
    r"motorloss_ftcboost_m2_eta0p497_b0_r(-?\d+)_(-?\d+)_(-?\d+)_(-?\d+)_(.+)_policy_refine_rep(\d+)\.csv"
)

for path in sorted(Path("logs").glob("motorloss_ftcboost_m2_eta0p497_b0_r*_policy_refine_rep*.csv")):
    ...
    rows.append({
        "controller": "event_triggered_policy_refine",
        "motor": 2,
        "eta": 0.497,
        "boost": 0,
        "policy": policy,
...
if out.empty:
    raise SystemExit("[ERROR] No motor-2 policy refinement logs found.")

out = out.sort_values(["policy", "rep"])

out_path = Path("results/tables/ftcresid_m2_eta0p497_policy_refine_summary.csv")
out_path.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(out_path, index=False)

print("\n[MOTOR 2 ETA=0.497 POLICY REFINEMENT]")
...
agg = agg.sort_values(
    ["safe_count", "max_vz", "mean_vz", "max_drift"],
    ascending=[False, True, True, True],
)

agg_path = Path("results/tables/ftcresid_m2_eta0p497_policy_refine_aggregate.csv")
...
plt.xticks(range(len(agg)), labels, rotation=55, ha="right")
plt.ylabel("Mean first-contact vertical speed [m/s]")
plt.title("Motor 2 eta=0.497 policy refinement")
plt.grid(True, axis="y")
plt.legend()

fig_path = Path("results/figures/ftcresid_m2_eta0p497_policy_refine.png")
