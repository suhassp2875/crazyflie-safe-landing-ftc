...
pat = re.compile(
    r"motorloss_ftcboost_m3_eta0p497_b0_r(-?\d+)_(-?\d+)_(-?\d+)_(-?\d+)_(.+)_rep(\d+)\.csv"
)

for path in sorted(Path("logs").glob("motorloss_ftcboost_m3_eta0p497_b0_r*_rep*.csv")):
...
if out.empty:
    raise SystemExit("[ERROR] No motor 3 tune logs found.")

out = out.sort_values(["pattern", "rep"])

out_path = Path("results/tables/ftcresid_m3_eta0p497_tune_repeats_summary.csv")
out_path.parent.mkdir(parents=True, exist_ok=True)
out.to_csv(out_path, index=False)

print("\n[M3 ETA=0.497 RESIDUAL TUNING REPEATS]")
print(out.to_string(index=False))

agg = out.groupby(["pattern", "r1", "r2", "r3", "r4"]).agg(
    n=("rep", "count"),
...
agg = agg.sort_values(["safe_count", "mean_vz", "max_drift"], ascending=[False, True, True])

agg_path = Path("results/tables/ftcresid_m3_eta0p497_tune_repeats_aggregate.csv")
...
plt.xticks(range(len(agg)), labels, rotation=65, ha="right")
plt.ylabel("Mean first-contact vertical speed [m/s]")
plt.title("Motor 3 eta=0.497 residual tuning")
plt.grid(True, axis="y")
plt.legend()

fig_path = Path("results/figures/ftcresid_m3_eta0p497_tune_repeats.png")
