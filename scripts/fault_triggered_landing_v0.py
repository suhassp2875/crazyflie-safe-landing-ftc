...
LOG_PATH = PROJECT_ROOT / "logs" / "fault_triggered_landing_v0.csv"
...
def find_touchdown_row(rows):
    # Use first ground-contact row during emergency landing, not final resting row.
    for row in rows:
        if row.get("phase") == "emergency_landing" and finite_row(row):
            if row["z"] <= GROUND_CONTACT_Z_M:
                return row

    # Fallback: last finite emergency row.
    for row in reversed(rows):
        if row.get("phase") == "emergency_landing" and finite_row(row):
            return row

    # Fallback: last finite row overall.
    for row in reversed(rows):
        if finite_row(row):
            return row
